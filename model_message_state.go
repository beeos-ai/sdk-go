...
// MessageState Lifecycle state of a v3 message envelope. `streaming` is the sole non-terminal state; everything else is immutable.  State transitions:   - `streaming -> completed`  (normal LLM/agent finish)   - `streaming -> failed`     (transport / model error)   - `streaming -> refused`    (content filter / agent decline)   - `streaming -> cancelled`  (user_stop / timeout) Once terminal, the row is immutable and any PATCH returns `410 Gone` with `code = message_terminal`. 
type MessageState string

// List of MessageState
const (
	MESSAGESTATE_STREAMING MessageState = "streaming"
	MESSAGESTATE_COMPLETED MessageState = "completed"
	MESSAGESTATE_FAILED MessageState = "failed"
	MESSAGESTATE_REFUSED MessageState = "refused"
	MESSAGESTATE_CANCELLED MessageState = "cancelled"
)
