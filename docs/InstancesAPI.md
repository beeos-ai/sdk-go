...
[**GetDeviceInfo**](InstancesAPI.md#GetDeviceInfo) | **Get** /api/v1/instances/{id}/device/info | BYOD device registration / static metadata.
...
> GetDeviceInfo200Response GetDeviceInfo(ctx, id).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.InstancesAPI.GetDeviceInfo(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `InstancesAPI.GetDeviceInfo``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetDeviceInfo`: GetDeviceInfo200Response
	fmt.Fprintf(os.Stdout, "Response from `InstancesAPI.GetDeviceInfo`: %v\n", resp)
}
```
...
[**GetDeviceInfo200Response**](GetDeviceInfo200Response.md)
