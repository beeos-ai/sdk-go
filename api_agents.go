...
/*
DeleteAgent Request deletion of an owned agent.

This compatibility endpoint deliberately fails closed with `409`.
It verifies ownership but does not delete data until the durable
runtime `agent/delete` projection path is wired. It therefore has
no successful response by design.
...
/*
GetAgent Get an owned agent by ID.
...
func (r ApiListAgentsRequest) InstanceId2(instanceId2 string) ApiListAgentsRequest {
...
func (a *AgentsAPIService) ListAgentsExecute(r ApiListAgentsRequest) (*AgentListResponse, *http.Response, error) {
	...
	if r.instanceId2 != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "instanceId", r.instanceId2, "form", "")
	}
	if r.status != nil {
...
/*
UpdateAgent Update mutable agent fields (visibility / mcpEnabled).
...
Authorisation: caller MUST own the agent. Public discovery uses the
agent-card endpoints; this management resource never bypasses owner
scope. Patches with no recognised fields fail with
`invalid_param`.
