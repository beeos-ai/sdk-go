...
type Error struct {
	// RFC 9457 problem-type slug. Most SDK consumers key on `code` rather than `type`; this field is kept for structural parity with the wider apierror catalogue. 
	Type *string `json:"type,omitempty"`
	// Stable machine-readable error code. See [docs/reference/errors.md](https://github.com/beeos-ai/beeos/blob/main/docs/reference/errors.md) for the complete catalogue. 
	Code string `json:"code"`
	// Human-readable explanation; presentation-only, never key on this.
	Message string `json:"message"`
	// Per-request correlation id propagated end-to-end (also returned in the `X-Request-Id` response header). Include this value when reporting errors so the server-side trace can be located. 
	RequestId *string `json:"request_id,omitempty"`
	// Field path of the offending parameter for validation-style errors (`code` in `missing_param` / `invalid_param`). Empty for errors that don't reference a specific field. 
	Param *string `json:"param,omitempty"`
	// Optional structured key-value hints for the client (retry strategy, related resource ids, etc.). Always a string-to-string map. 
